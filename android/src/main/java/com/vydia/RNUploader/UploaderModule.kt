...
class UploaderModule(val reactContext: ReactApplicationContext) : ReactContextBaseJavaModule(reactContext), LifecycleEventListener {
    ...
    private fun configureUploadServiceHTTPStack(options: ReadableMap, promise: Promise) {
        ...
        if (BuildConfig.DEBUG) {
            ...
            // Set up proxy and authenticator
            val proxy = Proxy(Proxy.Type.HTTP, InetSocketAddress(BackGroundUploadConstant.getProxy(), BackGroundUploadConstant.getPort()))
            val proxyAuthenticator = Authenticator { route: Route?, response: Response ->
                val credential = basic(BackGroundUploadConstant.getUsername(), BackGroundUploadConstant.getPassword())
                response.request.newBuilder()
                        .header("Proxy-Authorization", credential)
                        .build()
            }

            // Create a new ReactCookieJarContainer
            val cookieJar: CookieJar = ReactCookieJarContainer()
            // Set up SSL context
